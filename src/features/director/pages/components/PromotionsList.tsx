...
function discountLabel(p: Promotion) {
  switch (p.type) {
    case "percentage":
      return p.percent !== undefined ? `-${p.percent}%` : "—";
    case "beforeAfter":
      return p.beforePrice !== undefined && p.afterPrice !== undefined
        ? `${p.beforePrice}→${p.afterPrice} Bs`
        : "—";
    case "twoForOne":
      return "2×1";
    case "tieredByCount":
      if (!p.tiers || p.tiers.length === 0) return "—";
      const sorted = p.tiers.slice().sort((a, b) => a.count - b.count);
      return sorted.map((t) => `${t.count}→${t.price}`).join(" / ") + " Bs";
    default:
      return "—";
  }
}
...
function BadgeApplies({ to }: { to: AppliesTo }) {
  const cfg: Record<AppliesTo, { label: string; className: string }> = {
    regulares: { label: "Regulares", className: "bg-femme-softyellow/70 text-ink" },
    talleres: { label: "Talleres", className: "bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300" },
    intensivos: { label: "Intensivos", className: "bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-300" },
    especiales: { label: "Especiales", className: "bg-pink-100 text-pink-800 dark:bg-pink-900/30 dark:text-pink-300" },
  };
  const { label, className } = cfg[to];
  return (
    <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ring-1 ring-black/5 ${className}`}>
      {label}
    </span>
  );
}
          ...
          <tbody className="divide-y divide-gray-200/70 text-sm dark:divide-white/10">
            {filtered.map((p) => (
              <tr key={p.id} className="align-top">
                ...
                <td className="px-4 py-3">
                  <BadgeApplies to={p.appliesTo} />
                </td>
                <td className="px-4 py-3">
                  {p.appliesTo === "regulares" && p.packages?.length ? (
                    <div className="flex flex-wrap gap-1">
                      {p.packages.map((pkg) => (
                        <span
                          key={pkg}
                          className="rounded-md bg-femme-softyellow/60 px-2 py-0.5 text-[11px] font-medium text-ink ring-1 ring-black/5"
                        >
                          {pkgLabel(pkg)}
                        </span>
                      ))}
                    ...
                    <div className="flex flex-wrap items-center gap-1 text-xs text-graphite/80">
                      <span>{appliesToLabel(p.appliesTo)}</span>
                      {p.targetLabel && (
                        <span className="rounded-md bg-gray-100 px-1.5 py-0.5 text-[11px] font-medium text-ink ring-1 ring-black/5 dark:bg-white/10">
                          {p.targetLabel}
                        </span>
                      )}
                    </div>
