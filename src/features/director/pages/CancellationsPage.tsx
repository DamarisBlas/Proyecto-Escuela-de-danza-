...
export default function CancellationsPage() {
  ...
  const selectedLabel = selected 
    ? `${selected.estilo.nombre_estilo} ${selected.hora_inicio}–${selected.hora_fin} ${selected.profesor.nombre} ${selected.profesor.apellido} (${selected.cancelado ? 'CANCELADA' : 'ACTIVA'})`
    : ''
              ...
              {loadingClasses ? (
                ...
                <select value={form.classId} onChange={(e) => update('classId', e.target.value)} className={`w-full rounded-xl border px-3 py-2 bg-white outline-none focus:ring-4 ${errors.classId ? 'border-red-400 focus:ring-red-100' : 'border-neutral-300 focus:ring-neutral-200'}`}>
                  ...
                  {classes.map((sesion: any) => {
                    const label = `${sesion.estilo.nombre_estilo} ${sesion.hora_inicio}–${sesion.hora_fin} ${sesion.profesor.nombre} ${sesion.profesor.apellido}${sesion.cancelado ? ' (CANCELADA)' : ''}`
                    return (
